...
- **Watch live updates on matchday** (`good first issue`) — the updater is wired to TheSportsDB's FIFA World Cup feed (league 4429), verified against the real schedule + team names. On the first live match, just confirm a real in-play score actually flows through to `matches.json` (we couldn't test an in-play score pre-tournament).
- **Bracket predictor — share image** (`feature`) — the interactive predictor ships at `/predict` ([components/BracketSimulator.tsx](../components/BracketSimulator.tsx)): pick winners through the tree, picks propagate via the `W##`/`L##` graph, and the bracket is shareable via the URL hash. Nice-to-have left: render a shareable result *image* (the current share is a link).
